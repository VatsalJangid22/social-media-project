...
export const likePost = async (req, res) => {
    try {
        ...
        //like logic started
        await post.updateOne({$addToSet:{likes: likeKrneVala}});
        await post.save();

        const user = await User.findById(likeKrneVala).select("username profilePicture");

        const ownerId = post.author.toString();

        if(ownerId !== likeKrneVala){
            const notification = {
                type:"like",
                userId:likeKrneVala,
                userDetails: user,
            ...
            const postOwnerSocketId = getUserSocketId(ownerId);
...
export const dislikePost = async (req, res) => {
    try {
        ...
        //like logic started
        await post.updateOne({$pull:{likes: likeKrneVala}});
        await post.save();
